...
const MonthNav = memo(({ year, month, onPrev, onNext, onToday }) => (
  <div className="flex items-center gap-2">
    <button onClick={onPrev} className="p-1.5 rounded-lg active:opacity-60" style={{ color: "var(--ink-muted)", background: "var(--surface-2)" }}>
      <Icons.chevronLeft />
    </button>
    <div className="text-sm font-bold min-w-[110px] text-center" style={{ color: "var(--ink)" }}>
      {MONTH_NAMES[month]} {year}
    </div>
    <button onClick={onNext} className="p-1.5 rounded-lg active:opacity-60" style={{ color: "var(--ink-muted)", background: "var(--surface-2)" }}>
      <Icons.chevronRight />
    </button>
    {!(year === CURRENT_YEAR && month === new Date().getMonth()) && (
      <button onClick={onToday} className="text-[10px] font-bold px-2 py-1 rounded-md ml-1" style={{ color: "var(--accent)", border: "1px solid var(--accent)" }}>
        Today
      </button>
    )}
  </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {parseFloat(extraHours) > 0 && (
        <div>
          ...
          <input type="text" value={extraNote} onChange={(e) => setExtraNote(e.target.value)} placeholder="Why extra hours?" required
            className="w-full text-base p-3 rounded-xl"
            style={{ border: "1px solid var(--stage-contract)", background: "var(--stage-contract)10", color: "var(--ink)" }} />
        </div>
      )}
...
const WorkerCard = memo(function WorkerCard({ worker, vKey, orders, onOpen, onAttendanceChange, isSaving }) {
  ...
  const monthlyEarnings = useMemo(() => {
    ...
    return Math.round(hours * (worker.hourlyRate || 0));
  }, [worker, vKey]);

  return (
    <div onClick={() => onOpen(worker.id)} className="p-4 rounded-2xl active:scale-[0.98] transition-transform cursor-pointer"
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shrink-0 relative"
          ...
          {worker.initials}
          {isSaving && <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full animate-pulse" style={{ background: "var(--accent)" }} />}
        </div>
...
/* ─── WeekStrip ─── */
const WeekStrip = memo(function WeekStrip({ year, weekIndex, yearWeeks, currentWeek, selected, onYearChange, onPrevWeek, onNextWeek, onJumpToToday, onAttendanceClick, weekRangeLabel, isSaving }) {
  return (
    <div className="mb-5">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => onPrevWeek()} disabled={weekIndex === 0} className="p-1 rounded-lg disabled:opacity-30" style={{ color: "var(--ink-muted)" }}>
            <Icons.chevronLeft />
          </button>
          <span className="text-xs font-medium" style={{ color: "var(--ink-muted)" }}>Week {weekIndex + 1}</span>
          <button onClick={() => onNextWeek()} disabled={weekIndex === yearWeeks.length - 1} className="p-1 rounded-lg disabled:opacity-30" style={{ color: "var(--ink-muted)" }}>
            <Icons.chevronRight />
          </button>
        </div>
      </div>
      <div className="text-xs font-medium mb-3 text-center" style={{ color: "var(--accent)" }}>
        {weekRangeLabel}
        {isSaving && <span className="ml-2 opacity-70">(saving...)</span>}
      </div>
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "var(--bg)" }}>
      ...
      <div className="mx-4 mb-3 p-3 rounded-xl shrink-0" style={{ background: "var(--accent-soft)", border: "1px solid rgba(254,189,17,0.15)" }}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            {isSaving && <span className="text-[10px] animate-pulse" style={{ color: "var(--accent)" }}>(saving...)</span>}
          </div>
      ...
      <div className="flex gap-2 px-4 mb-3 overflow-x-auto no-scrollbar shrink-0">
        {roles.map((r) => (
          <button key={r} onClick={() => setRoleFilter(r)} className="text-xs font-bold px-3 py-2 rounded-lg whitespace-nowrap"
            style={{ background: roleFilter === r ? "var(--surface-2)" : "var(--surface)", color: roleFilter === r ? "var(--ink)" : "var(--ink-muted)", border: `1px solid ${roleFilter === r ? "var(--border)" : "transparent"}` }}>
            {r}
          </button>
        ))}
      </div>
        ...
        {["ACTIVE", "OFF"].map((s) => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(statusFilter === s ? "All" : s)} className="text-xs font-bold px-3 py-2 rounded-lg flex items-center gap-1.5"
              style={{ background: active ? "var(--surface-2)" : "var(--surface)", color: active ? "var(--ink)" : "var(--ink-muted)", border: `1px solid ${active ? "var(--border)" : "transparent"}` }}>
...
const DetailScreen = memo(function DetailScreen({
  selected, activeTab, setActiveTab, onBack, year, weekIndex, yearWeeks, currentWeek,
  ...
  return (
    <div className="flex flex-col h-full screen-enter" style={{ background: "var(--bg)" }}>
      <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="text-base font-bold truncate" style={{ color: "var(--ink)" }}>{selected.full_name || `${selected.firstName} ${selected.lastName}`}</div>
          <div className="text-xs" style={{ color: "var(--ink-muted)" }}>id-{selected.id}  {selected.role}</div>
        </div>
      ...
      <div className="flex items-center shrink-0" style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)} className="flex-1 flex flex-col items-center gap-1 py-3 text-xs font-bold transition-colors relative"
      ...
      <div data-detail-scroll className="flex-1 overflow-y-auto no-scrollbar pb-24">
        {activeTab === "attendance" && (
          <div className="p-4">
            <WeekStrip
              year={year} weekIndex={weekIndex} yearWeeks={yearWeeks}
              currentWeek={currentWeek} selected={selected}
              onYearChange={onYearChange} onPrevWeek={onPrevWeek}
              onNextWeek={onNextWeek} onJumpToToday={onJumpToToday}
              ...
              <div className="mt-4 flex items-center gap-3 text-[10px]" style={{ color: "var(--ink-muted)" }}>
                {ATTENDANCE_OPTIONS.map((opt) => (
                  <span key={opt} className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full" style={{ background: ATTENDANCE_COLORS[opt] }} />
          ...
          <div className="p-4 space-y-4">
            <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={onAddPayment} className="text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1" style={{ background: "var(--accent)", color: "#fff" }}>
                  <Icons.plus /> Add
                </button>
              </div>
              <div className="space-y-2">
...
export default function WorkersApp({ workersData, orders = [] }) {
  ...
  const [weekIndex, setWeekIndexState] = useState(() => {
    ...
    return weeks.findIndex((w) => w.includes(TODAY)) || 0;
  });
  ...
  /* ─── Debounced Batch Update for Attendance ─── */
  const flushAttendanceToDB = useCallback(async (changes) => {
  ...
  const { addChange, isSaving } = useAttendanceBatchUpdates(flushAttendanceToDB, 1000);

  /* ─── Modified setAttendance: updates UI instantly, queues DB save ─── */
  const setAttendance = useCallback((workerId, date, status) => {
    ...
    // 2. Queue for debounced batch save to DB
    addChange({ workerId, date, status });
  }, [addChange]);
  ...
  const roles = useMemo(() => ["All", ...new Set(workersData.map((w) => w.role).filter(Boolean))], [workersData]);
  ...
  const goPrevMonth = useCallback(() => {
    setViewMonthState((m) => { if (m === 0) { setViewYearState((y) => y - 1); return 11; } return m - 1; });
  }, []);
  const goNextMonth = useCallback(() => {
    setViewMonthState((m) => { if (m === 11) { setViewYearState((y) => y + 1); return 0; } return m + 1; });
  }, []);
  const goCurrentMonth = useCallback(() => { setViewMonthState(new Date().getMonth()); setViewYearState(CURRENT_YEAR); }, []);
  ...
  return (
    <div className="h-screen w-full flex flex-col lg:flex-row overflow-hidden" style={{ background: "var(--bg)" }}>
      ...
      <div className={`${screen === "list" ? "flex" : "hidden"} lg:flex w-full lg:w-96 xl:w-[620px] flex-col shrink-0 h-full border-b lg:border-b-0 lg:border-r`}>
        <ListScreen
          workers={workers} filtered={filtered} search={search} setSearch={setSearch}
          roleFilter={roleFilter} setRoleFilter={setRoleFilter} statusFilter={statusFilter}
          setStatusFilter={setStatusFilter} roles={roles} todayPresent={todayPresent}
      ...
      {showTimeEntryModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="w-full max-w-md p-5 rounded-t-3xl sm:rounded-3xl max-h-[90vh] overflow-y-auto"
            style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="w-full max-w-md p-5 rounded-t-3xl sm:rounded-3xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
            <h3 className="text-lg font-bold mb-4" style={{ color: "var(--ink)" }}>Add Payment</h3>
