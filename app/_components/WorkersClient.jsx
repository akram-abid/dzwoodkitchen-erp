...
const WorkerCard = memo(function WorkerCard({
  ...
  const todayColor = todayStatus
    ? ATTENDANCE_COLORS[todayStatus]
...
export default function WorkersApp({ orders = [] }) {
  ...
  const weekRangeLabel = useMemo(
    () => getWeekRangeLabel(currentWeek),
    [currentWeek],
  );
  const roles = useMemo(
    () => ["All", ...new Set(WORKERS.map((w) => w.role))],
    [],
  );
  ...
  const todayPresent = useMemo(
    () => workers.filter((w) => w.attendance[TODAY] === "PRESENT").length,
    [workers],
  );
  const todayAbsent = useMemo(
    () =>
      workers.filter((w) => ["ABSENT", "SICK"].includes(w.attendance[TODAY]))
        .length,
    [workers],
  );
  const todayNotSet = useMemo(
    () => workers.filter((w) => !w.attendance[TODAY]).length,
    [workers],
  );
