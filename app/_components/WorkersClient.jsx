...
const CURRENT_MONTH = TODAY.slice(0, 7); // "2026-07"
...
const cycleStatus = (current) => {
  if (!current || current === 'NOT SET') return 'PRESENT';
  const idx = ATTENDANCE_OPTIONS.indexOf(current);
  if (idx === -1) return 'PRESENT';
  if (idx === ATTENDANCE_OPTIONS.length - 1) return undefined; // back to unset
...
const SkillTag = ({ skill }) => {
  ...
  return (
    <span className="text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded" style={{ background: `${c}18`, color: c }}>
      {skill}
    </span>
  );
};
...
const AttendanceDot = ({ status }) => {
  ...
  return (
    <span className="flex items-center gap-1.5">
      <span className="w-3 h-3 rounded-full" style={{ background: s.color, opacity: status ? 1 : 0.35 }} />
      <span className="text-sm font-medium" style={{ color: s.color, opacity: status ? 1 : 0.7 }}>{s.label}</span>
    </span>
  );
};
...
const WORKERS = [
  ...
  },
  {
    ...
    payments: [
      { date: '2026-07-05', amount: 25000, note: 'Weekly advance' },
    ],
  ...
  },
  {
    ...
    payments: [
      { id: 'P-001', date: '2026-07-01', amount: 40000, note: 'Monthly advance' },
    ],
...
export default function WorkersClient() {
  ...
  const getMonthlyMeters = (w) => {
    return w.assignments
      ?.filter(a => a.date.startsWith(CURRENT_MONTH))
      ?.reduce((sum, a) => sum + a.meters, 0) || 0;
  };

  const getMonthlyHours = (w) => {
    return w.timeEntries
  ...
  const getMonthlyEarnings = (w) => {
    if (w.paymentType === 'meters') {
      return Math.round(getMonthlyMeters(w) * w.meterRate);
    }
    ...
    <div className="flex h-full" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden" style={{ background: 'var(--surface)' }}>
            ...
            {['ACTIVE', 'OFF'].map(s => {
              ...
              return (
                ...
                >
                  ...
                  <span className="ml-1 px-1.5 py-0.5 rounded text-xs" style={{ background: 'var(--bg)', color: 'var(--ink-muted)' }}>{count}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex items-center gap-3 px-4 py-2 shrink-0" style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg)' }}>
          ...
          <span className="text-base font-bold" style={{ color: 'var(--ink)', minWidth: 50, textAlign: 'center' }}>{year}</span>
          <button 
            onClick={() => setYear(y => y + 1)}
            className="text-sm font-medium px-2 py-1 rounded"
            style={{ color: 'var(--ink-muted)', border: '1px solid var(--border)' }}
          ...
          <span className="text-sm font-medium" style={{ color: 'var(--ink)', minWidth: 140, textAlign: 'center' }}>
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-left text-sm">
            <thead className="sticky top-0 z-10" style={{ background: 'var(--surface-2)' }}>
              <tr>
                <th className="px-4 py-3 text-sm font-medium" style={{ color: 'var(--ink-muted)' }}>Worker</th>
                <th className="px-4 py-3 text-sm font-medium" style={{ color: 'var(--ink-muted)' }}>Role</th>
                <th className="px-4 py-3 text-sm font-medium" style={{ color: 'var(--ink-muted)' }}>
                  <div className="flex gap-1">
                    {currentWeek.map(d => (
                      <div key={d} className="w-9 text-center text-xs" style={{ color: d === TODAY ? 'var(--accent)' : 'var(--ink-muted)' }}>
                        {getWeekDayName(d)}
                      </div>
                    ))}
                  </div>
                </th>
                <th className="px-4 py-3 text-sm font-medium" style={{ color: 'var(--ink-muted)' }}>Payment</th>
                <th className="px-4 py-3 text-sm font-medium text-right" style={{ color: 'var(--ink-muted)' }}>Month Total</th>
                <th className="px-4 py-3 text-sm font-medium text-center" style={{ color: 'var(--ink-muted)' }}></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(w => {
                const isSel = selectedId === w.id;
                const earned = getMonthlyEarnings(w);
                return (
                  <tr
                    key={w.id}
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0" style={{ background: 'var(--surface-2)', color: 'var(--accent)' }}>
                          {w.initials}
                        </div>
                        <div>
                          <div className="text-base font-medium">{w.shortName}</div>
                          <div className="text-sm" style={{ color: 'var(--ink-muted)' }}>{w.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      ...
                      <div className="flex gap-1">
                        {currentWeek.map(d => {
                          ...
                          return (
                            <button
                              key={d}
                              className="w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold cursor-pointer transition-all"
                              style={{ 
                                background: status ? `${color}25` : 'var(--surface-2)', 
                                color: status ? color : 'var(--ink-muted)',
                                border: isToday ? `2px solid ${status ? color : 'var(--accent)'}` : `1px solid ${status ? color + '50' : 'var(--border)'}`,
                    ...
                    <td className="px-4 py-3 text-right font-bold tabular-nums text-base" style={{ color: 'var(--stage-completed)' }}>
                      {earned.toLocaleString()} DZD
                    </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-12 text-center text-base" style={{ color: 'var(--ink-muted)' }}>No workers match your filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* ─── Right: Detail Panel ─── */}
      <div className="w-[440px] shrink-0 flex flex-col overflow-y-auto" style={{ borderLeft: '1px solid var(--border)', background: 'var(--surface)' }}>
        
        {/* Header */}
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold" style={{ background: 'var(--surface-2)', color: 'var(--accent)' }}>
                {selected.initials}
              </div>
              <div>
                <h2 className="text-xl font-semibold" style={{ color: 'var(--ink)' }}>{selected.firstName} {selected.lastName}</h2>
        ...
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)', background: 'var(--accent-soft)' }}>
          <h3 className="text-sm font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--accent)' }}>
            {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })} — Salary
          </h3>
          <div className="grid grid-cols-3 gap-3 mb-3">
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--ink-muted)' }}>Earned</div>
              <div className="text-lg font-bold" style={{ color: 'var(--stage-completed)' }}>{monthlyEarned.toLocaleString()} DZD</div>
            </div>
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--ink-muted)' }}>Paid</div>
              <div className="text-lg font-bold" style={{ color: 'var(--stage-appointment)' }}>{monthlyPaid.toLocaleString()} DZD</div>
            </div>
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--surface)', border: `1px solid ${monthlyBalance >= 0 ? 'var(--stage-completed)' : 'var(--stage-contract)'}` }}>
              <div className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--ink-muted)' }}>{monthlyBalance >= 0 ? 'Remaining' : 'Overpaid'}</div>
              <div className="text-lg font-bold" style={{ color: monthlyBalance >= 0 ? 'var(--ink)' : 'var(--stage-contract)' }}>
        ...
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)', background: selected.paymentType === 'meters' ? 'var(--stage-appointment)08' : 'var(--stage-completed)08' }}>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: selected.paymentType === 'meters' ? 'var(--stage-appointment)' : 'var(--stage-completed)' }}>
              {selected.paymentType === 'meters' ? 'Meter-Based Payment' : 'Hourly Payment'}
            </h3>
            <span className="text-xl font-bold" style={{ color: selected.paymentType === 'meters' ? 'var(--stage-appointment)' : 'var(--stage-completed)' }}>
              {monthlyEarned.toLocaleString()} DZD
            </span>
          </div>
          <div className="text-sm" style={{ color: 'var(--ink-muted)' }}>
            {selected.paymentType === 'meters' 
              ? `${getMonthlyMeters(selected)} m this month @ ${selected.meterRate.toLocaleString()} DZD/m`
        ...
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
          <h3 className="text-sm font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--ink-muted)' }}>
            ...
            {currentWeek.map(d => {
              ...
              return (
                <div key={d} className="flex flex-col items-center gap-1">
                  <div className="text-xs font-medium" style={{ color: 'var(--ink-muted)' }}>{getWeekDayName(d)}</div>
                  <button
                    onClick={() => {
                      const next = cycleStatus(status);
                      setAttendance(selected.id, d, next);
                    }}
                  ...
                  <span className="text-xs font-medium" style={{ color: status ? color : 'var(--ink-muted)', opacity: status ? 1 : 0.5 }}>
                    {status || 'Not Set'}
                  </span>
                </div>
              );
        ...
        <div className="p-5 space-y-2" style={{ borderBottom: '1px solid var(--border)' }}>
          <h3 className="text-sm font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--ink-muted)' }}>Contact</h3>
          <div className="flex items-center gap-3 text-base p-2 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
        ...
        {selected.paymentType === 'meters' ? (
          <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--ink-muted)' }}>Assignments (Meters)</h3>
              <button 
            ...
            <div className="space-y-2">
              {selected.assignments?.filter(a => a.date.startsWith(CURRENT_MONTH)).length === 0 && (
                <div className="text-sm text-center py-4" style={{ color: 'var(--ink-muted)' }}>No assignments this month</div>
              )}
              {selected.assignments?.filter(a => a.date.startsWith(CURRENT_MONTH)).map(a => (
                <div key={a.id} className="p-3 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
                  <div className="flex items-center justify-between">
                    <span className="text-base font-semibold" style={{ color: 'var(--ink)' }}>{a.project}</span>
                    <button onClick={() => deleteAssignment(a.id)} style={{ color: 'var(--stage-contract)' }}><Icons.trash /></button>
                  </div>
                  <div className="flex items-center gap-3 mt-1 text-sm" style={{ color: 'var(--ink-muted)' }}>
                  ...
                  {a.note && <div className="text-sm mt-1" style={{ color: 'var(--ink-muted)' }}>{a.note}</div>}
                </div>
              ))}
            </div>
            <div className="mt-3 p-3 rounded-lg flex items-center justify-between" style={{ background: 'var(--stage-appointment)10' }}>
              <span className="text-sm font-medium" style={{ color: 'var(--stage-appointment)' }}>Total Meters This Month</span>
              <span className="text-base font-bold" style={{ color: 'var(--stage-appointment)' }}>{getMonthlyMeters(selected)} m</span>
            </div>
          </div>
        ) : (
          <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--ink-muted)' }}>Time Entries</h3>
              <button 
            ...
            <div className="space-y-2 max-h-64 overflow-auto">
              {selected.timeEntries?.filter(e => e.date.startsWith(CURRENT_MONTH)).length === 0 && (
                <div className="text-sm text-center py-4" style={{ color: 'var(--ink-muted)' }}>No entries this month</div>
              )}
              {selected.timeEntries?.filter(e => e.date.startsWith(CURRENT_MONTH)).sort((a, b) => b.date.localeCompare(a.date)).map(e => {
                const [inH, inM] = e.clockIn.split(':').map(Number);
                const [outH, outM] = e.clockOut.split(':').map(Number);
                const hours = (outH + outM / 60) - (inH + inM / 60);
                const dayPay = Math.round((hours + (e.extraHours || 0)) * selected.hourlyRate);
                return (
                  <div key={e.date} className="p-3 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
                    <div className="flex items-center justify-between">
                      <span className="text-base font-semibold" style={{ color: 'var(--ink)' }}>{e.date}</span>
                      <div className="flex gap-1">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-sm" style={{ color: 'var(--ink-muted)' }}>
                      ...
                      <span style={{ color: 'var(--stage-completed)' }}>{hours.toFixed(1)}h</span>
                    </div>
                    {e.extraHours > 0 && (
                      <div className="mt-2 p-2 rounded text-sm" style={{ background: 'var(--stage-contract)10', border: '1px solid var(--stage-contract)30' }}>
                        ...
                        <div className="font-medium mt-1" style={{ color: 'var(--stage-contract)' }}>
                          Extra: {Math.round(e.extraHours * selected.hourlyRate).toLocaleString()} DZD
                        </div>
                      </div>
                    )}
                    <div className="mt-2 text-sm font-bold text-right" style={{ color: 'var(--stage-completed)' }}>
                      {dayPay.toLocaleString()} DZD
                    </div>
                  </div>
                );
              })}
        ...
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--ink-muted)' }}>Payments Taken This Month</h3>
            <button 
          ...
          <div className="space-y-2">
            {selected.payments?.filter(p => p.date.startsWith(CURRENT_MONTH)).length === 0 && (
              <div className="text-sm text-center py-4" style={{ color: 'var(--ink-muted)' }}>No payments recorded</div>
            )}
            {selected.payments?.filter(p => p.date.startsWith(CURRENT_MONTH)).sort((a, b) => b.date.localeCompare(a.date)).map(p => (
              <div key={p.id} className="p-3 rounded-lg flex items-center justify-between" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
                <div>
                  <div className="text-base font-semibold" style={{ color: 'var(--ink)' }}>{p.amount.toLocaleString()} DZD</div>
                  <div className="text-sm" style={{ color: 'var(--ink-muted)' }}>{p.date} · {p.note}</div>
                </div>
                <button onClick={() => deletePayment(p.id)} style={{ color: 'var(--stage-contract)' }}><Icons.trash /></button>
              </div>
            ))}
          </div>
          <div className="mt-3 p-3 rounded-lg flex items-center justify-between" style={{ background: 'var(--accent-soft)' }}>
            <span className="text-sm font-medium" style={{ color: 'var(--accent)' }}>Total Paid</span>
        ...
        {/* Performance */}
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
          <h3 className="text-sm font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--ink-muted)' }}>Performance</h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
              ...
              <div className="text-xs uppercase tracking-wider mt-1" style={{ color: 'var(--ink-muted)' }}>Orders Done</div>
            </div>
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
              ...
              <div className="text-xs uppercase tracking-wider mt-1" style={{ color: 'var(--ink-muted)' }}>On-Time Rate</div>
            </div>
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
              ...
              <div className="text-xs uppercase tracking-wider mt-1" style={{ color: 'var(--ink-muted)' }}>Avg Quality</div>
            </div>
            <div className="p-3 rounded-lg text-center" style={{ background: 'var(--bg)', border: '1px solid var(--border)' }}>
              <div className="text-2xl font-bold" style={{ color: 'var(--stage-appointment)' }}>
                {selected.paymentType === 'meters' ? `${getMonthlyMeters(selected)}m` : `${getMonthlyHours(selected).toFixed(1)}h`}
              </div>
              <div className="text-xs uppercase tracking-wider mt-1" style={{ color: 'var(--ink-muted)' }}>
                {selected.paymentType === 'meters' ? 'This Month' : 'Hours This Month'}
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Notes */}
        <div className="p-5" style={{ borderBottom: '1px solid var(--border)' }}>
          <h3 className="text-sm font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--ink-muted)' }}>Notes</h3>
          <p className="text-base" style={{ color: 'var(--ink)' }}>{selected.notes}</p>
        </div>

        {/* Actions */}
        <div className="p-5 mt-auto space-y-2">
          <button className="w-full justify-center text-base font-medium px-4 py-2 rounded-lg flex items-center gap-2" style={{ background: 'var(--accent)', color: '#fff' }}>
            <Icons.briefcase /> Assign to Order
          </button>
          <div className="flex gap-2">
            <button className="flex-1 justify-center text-base font-medium px-4 py-2 rounded-lg flex items-center gap-2" style={{ background: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--ink)' }}>
      ...
      {showTimeEntryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-[420px] p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      {showAssignmentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-[420px] p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <h3 className="text-base font-bold mb-4" style={{ color: 'var(--ink)' }}>Add Assignment</h3>
            <AssignmentForm 
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
          <div className="w-[420px] p-5 rounded-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <h3 className="text-base font-bold mb-4" style={{ color: 'var(--ink)' }}>Record Payment</h3>
            <PaymentForm 
              onSave={addPayment} 
...
function TimeEntryForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Date</label>
        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Clock In</label>
          <input type="time" value={clockIn} onChange={e => setClockIn(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
        </div>
        <div>
          <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Clock Out</label>
          <input type="time" value={clockOut} onChange={e => setClockOut(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
        </div>
      </div>
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Extra Hours</label>
        <input type="number" step="0.5" min="0" value={extraHours} onChange={e => setExtraHours(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} />
      </div>
      {parseFloat(extraHours) > 0 && (
        <div>
          <label className="text-sm font-medium block mb-1" style={{ color: 'var(--stage-contract)' }}>Extra Hours Note *</label>
          <input type="text" value={extraNote} onChange={e => setExtraNote(e.target.value)} placeholder="Why extra hours?" className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--stage-contract)', background: 'var(--stage-contract)08', color: 'var(--ink)' }} required={parseFloat(extraHours) > 0} />
        </div>
      )}
      <div className="flex gap-2 mt-4">
        <button type="button" onClick={onCancel} className="flex-1 text-base py-2 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--ink)' }}>Cancel</button>
        <button type="submit" className="flex-1 text-base py-2 rounded-lg text-white" style={{ background: 'var(--stage-completed)' }}>Save</button>
      </div>
    </form>
  );
}
...
function AssignmentForm({ onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Project</label>
        <input type="text" value={project} onChange={e => setProject(e.target.value)} placeholder="Project name" className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
      </div>
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Meters Worked</label>
        <input type="number" step="0.1" min="0" value={meters} onChange={e => setMeters(e.target.value)} placeholder="e.g. 12.5" className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
      </div>
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Date</label>
        <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
      ...
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Note (optional)</label>
        <input type="text" value={note} onChange={e => setNote(e.target.value)} className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} />
      </div>
      <div className="flex gap-2 mt-4">
        <button type="button" onClick={onCancel} className="flex-1 text-base py-2 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--ink)' }}>Cancel</button>
        <button type="submit" className="flex-1 text-base py-2 rounded-lg text-white" style={{ background: 'var(--stage-appointment)' }}>Save</button>
      </div>
    </form>
  );
}
...
function PaymentForm({ onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Amount (DZD)</label>
        <input type="number" min="0" value={amount} onChange={e => setAmount(e.target.value)} placeholder="e.g. 50000" className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} required />
      </div>
      <div>
        <label className="text-sm font-medium block mb-1" style={{ color: 'var(--ink-muted)' }}>Note</label>
        <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="Advance, salary, etc." className="w-full text-base p-2 rounded-lg" style={{ border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--ink)' }} />
      </div>
      <div className="flex gap-2 mt-4">
        <button type="button" onClick={onCancel} className="flex-1 text-base py-2 rounded-lg" style={{ background: 'var(--bg)', border: '1px solid var(--border)', color: 'var(--ink)' }}>Cancel</button>
        <button type="submit" className="flex-1 text-base py-2 rounded-lg text-white" style={{ background: 'var(--accent)' }}>Save</button>
      </div>
    </form>
  );
}