...
const getWeekDayName = (dateStr) => new Date(dateStr).toLocaleDateString("en-US", { weekday: "short" });
...
const getMonthlyTimeEntries = (w, vKey) => (w.timeEntries || []).filter((e) => e.date.startsWith(vKey));

const getMonthlyHours = (w, vKey) => getMonthlyTimeEntries(w, vKey).reduce((sum, e) => sum + calcEntryHours(e), 0);

const getMonthlyMetersData = (w, vKey, orders) => {
  if (orders && orders.length > 0) {
    const workerOrders = orders.filter((o) => o.worker === w.shortName && o.created?.startsWith(vKey));
    const kitchens = workerOrders.map((o) => {
      const totalM = (o.items || []).reduce((sum, item) => sum + (Number(item.l) || 0) / 100, 0);
      return { name: o.project, orderId: o.id, meters: totalM, amount: Math.round(totalM * (w.meterRate || 0)) };
    });
    return { totalMeters: kitchens.reduce((s, k) => s + k.meters, 0), kitchens, source: "orders" };
  }
  const assignments = (w.assignments || []).filter((a) => a.date.startsWith(vKey));
  return {
    totalMeters: assignments.reduce((s, a) => s + a.meters, 0),
    kitchens: assignments.map((a) => ({ name: a.project, orderId: a.id, meters: a.meters, amount: Math.round(a.meters * (w.meterRate || 0)) })),
    source: "assignments",
  };
};

const getMonthlyEarnings = (w, vKey, orders) => {
  if (w.paymentType === "meters") return Math.round(getMonthlyMetersData(w, vKey, orders).totalMeters * w.meterRate);
  return Math.round(getMonthlyHours(w, vKey) * w.hourlyRate);
};

const getMonthlyPayments = (w, vKey) => (w.payments || []).filter((p) => p.date.startsWith(vKey)).reduce((s, p) => s + p.amount, 0);

const getBalance = (w, vKey, orders) => getMonthlyEarnings(w, vKey, orders) - getMonthlyPayments(w, vKey);
...
const StatusBadge = memo(({ status }) => (
  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md"
    style={{
      background: status === "ACTIVE" ? "var(--stage-completed)20" : "var(--ink-muted)20",
      color: status === "ACTIVE" ? "var(--stage-completed)" : "var(--ink-muted)",
...
const MonthNav = memo(({ year, month, onPrev, onNext, onToday }) => (
  <div className="flex items-center gap-2">
    <button onClick={onPrev} className="p-1.5 rounded-lg active:opacity-60"
      style={{ color: "var(--ink-muted)", background: "var(--surface-2)" }}>
      <Icons.chevronLeft />
    </button>
    <div className="text-sm font-bold min-w-[110px] text-center" style={{ color: "var(--ink)" }}>
      {MONTH_NAMES[month]} {year}
    </div>
    <button onClick={onNext} className="p-1.5 rounded-lg active:opacity-60"
      style={{ color: "var(--ink-muted)", background: "var(--surface-2)" }}>
      <Icons.chevronRight />
    </button>
    {!(year === CURRENT_YEAR && month === new Date().getMonth()) && (
      <button onClick={onToday} className="text-[10px] font-bold px-2 py-1 rounded-md ml-1"
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input type="date" value={date} onChange={(e) => setDate(e.target.value)} required
          ...
          style={{ border: `1px solid ${dateTaken ? "var(--stage-contract)" : "var(--border)"}`, background: "var(--bg)", color: "var(--ink)" }} />
        {dateTaken && <div className="text-xs mt-1.5" style={{ color: "var(--stage-contract)" }}>⚠ Entry already exists for this date.</div>}
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-sm font-medium block mb-1.5" style={{ color: "var(--ink-muted)" }}>Clock In</label>
          <input type="time" value={clockIn} onChange={(e) => setClockIn(e.target.value)} required
            className="w-full text-base p-3 rounded-xl" style={{ border: "1px solid var(--border)", background: "var(--bg)", color: "var(--ink)" }} />
        </div>
        <div>
          <label className="text-sm font-medium block mb-1.5" style={{ color: "var(--ink-muted)" }}>Clock Out</label>
          <input type="time" value={clockOut} onChange={(e) => setClockOut(e.target.value)} required
            className="w-full text-base p-3 rounded-xl" style={{ border: "1px solid var(--border)", background: "var(--bg)", color: "var(--ink)" }} />
        </div>
      </div>
      <div>
        <label className="text-sm font-medium block mb-1.5" style={{ color: "var(--ink-muted)" }}>Extra Hours</label>
        <input type="number" step="0.5" min="0" value={extraHours} onChange={(e) => setExtraHours(e.target.value)}
          className="w-full text-base p-3 rounded-xl" style={{ border: "1px solid var(--border)", background: "var(--bg)", color: "var(--ink)" }} />
      </div>
      {parseFloat(extraHours) > 0 && (
        <div>
          <label className="text-sm font-medium block mb-1.5" style={{ color: "var(--stage-contract)" }}>Extra Hours Note *</label>
          <input type="text" value={extraNote} onChange={(e) => setExtraNote(e.target.value)} placeholder="Why extra hours?" required={parseFloat(extraHours) > 0}
            className="w-full text-base p-3 rounded-xl" style={{ border: "1px solid var(--stage-contract)", background: "var(--stage-contract)10", color: "var(--ink)" }} />
        </div>
      )}
...
const FAKE_WORKERS = [
  {
    ...
    assignments: [
      ...
      { id: "A-103", project: "Office shelves", meters: 5.0, date: "2026-06-15", note: "Last month" },
    ],
    payments: [
      { id: "P-003", date: "2026-07-05", amount: 50000, note: "Advance" },
      { id: "P-004", date: "2026-07-01", amount: 30000, note: "Start of month" },
    ],
  ...
  },
  {
    ...
    payments: [{ id: "P-005", date: "2026-07-05", amount: 25000, note: "Weekly advance" }],
  ...
  },
  {
    ...
    payments: [{ id: "P-002", date: "2026-07-03", amount: 35000, note: "Mid-month" }],
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-base font-bold truncate" style={{ color: "var(--ink)" }}>{worker.shortName}</span>
        ...
        <div className="text-right shrink-0">
          <div className="text-sm font-bold tabular-nums" style={{ color: "var(--stage-completed)" }}>
            {monthlyEarnings.toLocaleString()} <span className="text-[10px]">DZD</span>
          </div>
          <div className="text-[10px]" style={{ color: "var(--ink-muted)" }}>this month</div>
        </div>
      </div>
...
const SalarySummary = memo(function SalarySummary({ selected, monthlyEarned, monthlyPaid, monthlyBalance, viewMonth, viewYear, monthlyMetersData, onPrevMonth, onNextMonth, onTodayMonth }) {
  return (
    <div className="p-4 rounded-2xl" style={{ background: "var(--accent-soft)", border: "1px solid rgba(254,189,17,0.15)" }}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="p-3 rounded-xl text-center" style={{ background: "var(--surface)", border: `1px solid ${monthlyBalance >= 0 ? "var(--stage-completed)" : "var(--stage-contract)"}` }}>
          ...
          <div className="text-base font-bold" style={{ color: monthlyBalance >= 0 ? "var(--ink)" : "var(--stage-contract)" }}>
      ...
      <div className="h-2 w-full rounded-full overflow-hidden" style={{ background: "var(--surface-2)" }}>
        <div className="h-full rounded-full" style={{
          ...
          background: monthlyPaid >= monthlyEarned ? "var(--stage-completed)" : "var(--accent)",
        }} />
      </div>
    </div>
  );
...
const WeekStrip = memo(function WeekStrip({ year, weekIndex, yearWeeks, currentWeek, selected, onYearChange, onPrevWeek, onNextWeek, onJumpToToday, onAttendanceClick, weekRangeLabel }) {
  return (
    <div className="mb-5">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <button onClick={() => onYearChange(year - 1)} className="text-xs font-bold px-2 py-1 rounded-md"
            style={{ color: "var(--ink-muted)", border: "1px solid var(--border)" }}>← Year</button>
          <span className="text-sm font-bold" style={{ color: "var(--ink)" }}>{year}</span>
          <button onClick={() => onYearChange(year + 1)} className="text-xs font-bold px-2 py-1 rounded-md"
            style={{ color: "var(--ink-muted)", border: "1px solid var(--border)" }}>Year →</button>
        </div>
        <div className="flex items-center gap-1">
          <button onClick={() => onPrevWeek()} disabled={weekIndex === 0}
            className="p-1 rounded-lg disabled:opacity-30" style={{ color: "var(--ink-muted)" }}><Icons.chevronLeft /></button>
          <span className="text-xs font-medium" style={{ color: "var(--ink-muted)" }}>Week {weekIndex + 1}</span>
          <button onClick={() => onNextWeek()} disabled={weekIndex === yearWeeks.length - 1}
            className="p-1 rounded-lg disabled:opacity-30" style={{ color: "var(--ink-muted)" }}><Icons.chevronRight /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {currentWeek.map((d) => {
          ...
          return (
            <button key={d} onClick={() => onAttendanceClick(selected.id, d, cycleStatus(status))}
              ...
              style={{
                ...
                border: isToday ? `2px solid var(--accent)` : `1px solid ${status ? color : "var(--border)"}`,
                opacity: status ? 1 : 0.7,
                minHeight: 64,
              }}>
              <span className="text-[10px] font-medium opacity-80">{getWeekDayName(d)}</span>
              <span className="text-lg font-bold">{new Date(d).getDate()}</span>
              <span className="text-[9px] font-bold uppercase tracking-wider">{status ? ATTENDANCE_LABELS[status] : "—"}</span>
            </button>
          );
        })}
      </div>
      <div className="flex justify-center mt-3">
        <button onClick={onJumpToToday} className="text-xs font-bold px-3 py-1.5 rounded-lg"
          style={{ color: "var(--accent)", border: "1px solid var(--accent)" }}>Jump to Today</button>
      </div>
    </div>
  );
});
...
const ListScreen = memo(function ListScreen({
  workers, filtered, search, setSearch, roleFilter, setRoleFilter, statusFilter, setStatusFilter,
  roles, todayPresent, todayAbsent, todayNotSet, vKey, orders,
  onMarkAll, onOpenWorker, onAttendanceChange,
}) {
  return (
      ...
      <div className="mx-4 mb-3 p-3 rounded-xl shrink-0" style={{ background: "var(--accent-soft)", border: "1px solid rgba(254,189,17,0.15)" }}>
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold" style={{ color: "var(--accent)" }}>Today</span>
          </div>
          <button onClick={onMarkAll} className="text-[10px] font-bold px-2.5 py-1.5 rounded-lg flex items-center gap-1"
        ...
        <div className="flex items-center gap-4 text-xs">
          <span className="flex items-center gap-1" style={{ color: "var(--ink)" }}>
            <span className="w-2 h-2 rounded-full" style={{ background: ATTENDANCE_COLORS.PRESENT }} /> {todayPresent} Present
          </span>
          <span className="flex items-center gap-1" style={{ color: "var(--ink)" }}>
            <span className="w-2 h-2 rounded-full" style={{ background: "#dc2626" }} /> {todayAbsent} Away
          </span>
          {todayNotSet > 0 && (
            <span className="flex items-center gap-1" style={{ color: "var(--ink)" }}>
              <span className="w-2 h-2 rounded-full" style={{ background: ATTENDANCE_COLORS["NOT SET"] }} /> {todayNotSet} Not Set
            </span>
          )}
        </div>
      </div>
      ...
      <div className="mx-4 mb-3 shrink-0">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search workers..."
            className="bg-transparent text-sm outline-none w-full placeholder:text-[var(--ink-muted)]" style={{ color: "var(--ink)" }} />
          {search && <button onClick={() => setSearch("")} style={{ color: "var(--ink-muted)" }}><Icons.x /></button>}
        </div>
      </div>
      ...
      <div className="flex gap-2 px-4 mb-3 overflow-x-auto no-scrollbar shrink-0">
        {roles.map((r) => (
          <button key={r} onClick={() => setRoleFilter(r)}
            ...
            style={{
              background: roleFilter === r ? "var(--surface-2)" : "var(--surface)",
        ...
        {["ACTIVE", "OFF"].map((s) => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(statusFilter === s ? "All" : s)}
              ...
              <span className="px-1.5 py-0.5 rounded text-[10px]" style={{ background: "var(--bg)", color: "var(--ink-muted)" }}>{count}</span>
            </button>
          );
        })}
      </div>

      <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-24 space-y-3 no-scrollbar">
        {filtered.map((w) => (
          <WorkerCard
            key={w.id}
            worker={w}
...
const DetailScreen = memo(function DetailScreen({
  selected, activeTab, setActiveTab, onBack,
  year, weekIndex, yearWeeks, currentWeek, viewMonth, viewYear, vKey, orders,
  onYearChange, onPrevWeek, onNextWeek, onJumpToToday, onPrevMonth, onNextMonth, onTodayMonth,
  onAttendanceClick, weekRangeLabel,
  monthlyEarned, monthlyPaid, monthlyBalance, monthlyEntries, monthlyMetersData,
  onAddTimeEntry, onEditTimeEntry, onDeleteTimeEntry, onPrint,
  ...
  return (
    <div className="flex flex-col h-full screen-enter" style={{ background: "var(--bg)" }}>
      <div className="flex items-center gap-3 px-4 py-3 shrink-0" style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
        <button onClick={onBack} className="lg:hidden p-2 rounded-xl active:opacity-60" style={{ color: "var(--ink)", background: "var(--surface-2)" }}>
          <Icons.arrowLeft />
        </button>
        <div className="flex-1 min-w-0">
          <div className="text-base font-bold truncate" style={{ color: "var(--ink)" }}>{selected.firstName} {selected.lastName}</div>
          <div className="text-xs" style={{ color: "var(--ink-muted)" }}>{selected.id} · {selected.role}</div>
        </div>
      ...
      <div className="flex items-center shrink-0" style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
        {[
          ...
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            ...
            style={{ color: activeTab === t.key ? "var(--accent)" : "var(--ink-muted)" }}>
            {t.icon}
            {t.label}
            {activeTab === t.key && <div className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full" style={{ background: "var(--accent)" }} />}
          </button>
        ))}
      </div>

      <div data-detail-scroll className="flex-1 min-h-0 overflow-y-auto no-scrollbar pb-24">
        {activeTab === "attendance" && (
          <div className="p-4">
            <WeekStrip
              year={year} weekIndex={weekIndex} yearWeeks={yearWeeks} currentWeek={currentWeek}
              selected={selected} onYearChange={onYearChange} onPrevWeek={onPrevWeek} onNextWeek={onNextWeek}
              onJumpToToday={onJumpToToday} onAttendanceClick={onAttendanceClick} weekRangeLabel={weekRangeLabel}
            />
            <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold uppercase tracking-wider" style={{ color: "var(--ink-muted)" }}>Calendar</h3>
                <MonthNav year={viewYear} month={viewMonth} onPrev={onPrevMonth} onNext={onNextMonth} onToday={onTodayMonth} />
              </div>
              ...
              <div className="grid grid-cols-7 gap-1">
                {getMonthGrid(viewYear, viewMonth).map((week, wi) =>
                  week.map((day, di) => {
                    ...
                    const color = status ? ATTENDANCE_COLORS[status] : ATTENDANCE_COLORS["NOT SET"];
                    return (
                      <button key={day.date} onClick={() => onAttendanceClick(selected.id, day.date, cycleStatus(status))}
                        ...
                        style={{
                          background: status ? color : "var(--surface-2)",
                          color: status ? "#fff" : "var(--ink-muted)",
                          border: isToday ? `2px solid ${status ? color : "var(--accent)"}` : `1px solid ${status ? color : "var(--border)"}`,
              ...
              <div className="mt-4 flex items-center gap-3 text-[10px]" style={{ color: "var(--ink-muted)" }}>
                {ATTENDANCE_OPTIONS.map((opt) => (
                  <span key={opt} className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-full" style={{ background: ATTENDANCE_COLORS[opt] }} />{ATTENDANCE_LABELS[opt]}
                  </span>
                ))}
              </div>
            </div>
        ...
        {activeTab === "work" && (
          <div className="p-4 space-y-4">
            <SalarySummary
              selected={selected} monthlyEarned={monthlyEarned} monthlyPaid={monthlyPaid}
              monthlyBalance={monthlyBalance} viewMonth={viewMonth} viewYear={viewYear}
              monthlyMetersData={monthlyMetersData}
              onPrevMonth={onPrevMonth} onNextMonth={onNextMonth} onTodayMonth={onTodayMonth}
            />

            <div className="p-4 rounded-2xl" style={{
              background: selected.paymentType === "meters" ? "var(--stage-appointment)08" : "var(--stage-completed)08",
              border: `1px solid ${selected.paymentType === "meters" ? "var(--stage-appointment)30" : "var(--stage-completed)30"}`,
            }}>
              <div className="flex items-center justify-between mb-1">
                <h3 className="text-xs font-bold uppercase tracking-wider"
                  style={{ color: selected.paymentType === "meters" ? "var(--stage-appointment)" : "var(--stage-completed)" }}>
                  {selected.paymentType === "meters" ? "Meter-Based" : "Hourly Payment"}
                </h3>
            ...
            {selected.paymentType === "hours" && (
              <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                ...
                {monthlyEntries.length === 0 ? (
                  <div className="text-sm text-center py-6 rounded-xl"
                    style={{ background: "var(--bg)", border: "1px dashed var(--border)", color: "var(--ink-muted)" }}>
                  ...
                  <div className="space-y-2">
                    {[...monthlyEntries].sort((a, b) => b.date.localeCompare(a.date)).map((e, idx) => {
                      ...
                      return (
                        <div key={`${e.date}-${idx}`} className="p-3 rounded-xl flex items-center justify-between"
                          ...
                          <div>
                            ...
                            <div className="text-xs" style={{ color: "var(--ink-muted)" }}>
                              {totalH.toFixed(1)}h {e.extraHours > 0 && <span style={{ color: "var(--stage-contract)" }}>+{e.extraHours}h extra</span>} · {e.extraNote || "Regular"}
                            </div>
                          </div>
                    ...
                    <div className="p-3 rounded-xl flex items-center justify-between mt-2"
                      style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}>
                      <span className="text-xs font-bold" style={{ color: "var(--ink-muted)" }}>Total ({monthlyEntries.length} days)</span>
                      <div className="flex items-center gap-4">
                        <span className="text-sm font-bold" style={{ color: "var(--ink)" }}>{getMonthlyHours(selected, vKey).toFixed(1)}h</span>
                        <span className="text-sm font-bold" style={{ color: "var(--stage-completed)" }}>{monthlyEarned.toLocaleString()} DZD</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {selected.paymentType === "meters" && (
              <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--ink-muted)" }}>Monthly Work</h3>
                  <div className="text-[10px] px-2 py-1 rounded font-bold"
                    style={{ background: "var(--stage-appointment)15", color: "var(--stage-appointment)" }}>
                    {monthlyMetersData.source === "orders" ? "Auto from orders" : "From records"}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                    <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: "var(--ink-muted)" }}>Total Meters</div>
                    <div className="text-xl font-bold" style={{ color: "var(--stage-appointment)" }}>{monthlyMetersData.totalMeters.toFixed(1)} m</div>
                  </div>
                  <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                    <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: "var(--ink-muted)" }}>Kitchens</div>
                    <div className="text-xl font-bold" style={{ color: "var(--stage-appointment)" }}>{monthlyMetersData.kitchens.length}</div>
                  </div>
                </div>
                {monthlyMetersData.kitchens.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {monthlyMetersData.kitchens.map((k, idx) => (
                      <div key={`${k.orderId}-${idx}`} className="p-3 rounded-xl flex items-center justify-between"
                        style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-bold truncate" style={{ color: "var(--ink)" }}>{k.name}</div>
                          <div className="text-[11px]" style={{ color: "var(--ink-muted)" }}>{k.orderId}</div>
                        </div>
                        <div className="text-right shrink-0">
                          <div className="text-sm font-bold" style={{ color: "var(--stage-appointment)" }}>{k.meters.toFixed(1)} m</div>
                          <div className="text-[11px] font-medium" style={{ color: "var(--stage-completed)" }}>{k.amount.toLocaleString()} DZD</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {activeTab === "payments" && (
          <div className="p-4 space-y-4">
            <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--ink-muted)" }}>Payments — {MONTH_NAMES[viewMonth]}</h3>
                <button onClick={onAddPayment} className="text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1"
                  style={{ background: "var(--accent)", color: "#fff" }}><Icons.plus /> Add</button>
              </div>
              <div className="space-y-2">
                {selected.payments?.filter((p) => p.date.startsWith(vKey)).length === 0 && (
        ...
        {activeTab === "info" && (
          <div className="p-4 space-y-4">
            ...
            <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <h3 className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: "var(--ink-muted)" }}>Performance</h3>
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                  <div className="text-xl font-bold" style={{ color: "var(--accent)" }}>{selected.performance.ordersCompleted}</div>
                  <div className="text-[10px] uppercase tracking-wider mt-1" style={{ color: "var(--ink-muted)" }}>Orders Done</div>
                </div>
                <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                  <div className="text-xl font-bold" style={{ color: selected.performance.onTimeRate >= 90 ? "var(--stage-completed)" : "var(--accent)" }}>{selected.performance.onTimeRate}%</div>
                  <div className="text-[10px] uppercase tracking-wider mt-1" style={{ color: "var(--ink-muted)" }}>On-Time Rate</div>
                </div>
                <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                  <div className="text-xl font-bold flex items-center justify-center gap-1" style={{ color: "var(--stage-production)" }}>
                    {selected.performance.avgQuality}<span style={{ color: "var(--accent)" }}><Icons.star /></span>
                  </div>
                  <div className="text-[10px] uppercase tracking-wider mt-1" style={{ color: "var(--ink-muted)" }}>Avg Quality</div>
                </div>
                <div className="p-3 rounded-xl text-center" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>
                  <div className="text-xl font-bold" style={{ color: "var(--stage-appointment)" }}>
                    {selected.paymentType === "meters" ? `${monthlyMetersData.totalMeters.toFixed(1)}m` : `${getMonthlyHours(selected, vKey).toFixed(1)}h`}
                  </div>
                  <div className="text-[10px] uppercase tracking-wider mt-1" style={{ color: "var(--ink-muted)" }}>{MONTH_NAMES[viewMonth]}</div>
                </div>
              </div>
            </div>
            <div className="p-4 rounded-2xl" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
              <h3 className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: "var(--ink-muted)" }}>Notes</h3>
              <p className="text-sm leading-relaxed" style={{ color: "var(--ink)" }}>{selected.notes}</p>
            </div>
            <div className="space-y-2">
              <button className="w-full justify-center text-sm font-bold px-4 py-3 rounded-xl flex items-center gap-2"
                style={{ background: "var(--accent)", color: "#000" }}>
                <Icons.briefcase /> Assign to Order
              </button>
              <div className="flex gap-2">
                <button className="flex-1 justify-center text-sm font-bold px-4 py-3 rounded-xl flex items-center gap-2"
...
export default function WorkersApp({ workersData, orders = [] }) {
  ...
  const monthlyBalance = useMemo(() => monthlyEarned - monthlyPaid, [monthlyEarned, monthlyPaid]);
  const monthlyEntries = useMemo(() => getMonthlyTimeEntries(selected, vKey), [selected, vKey]);
  const monthlyMetersData = useMemo(
    () => selected.paymentType === "meters" ? getMonthlyMetersData(selected, vKey, orders) : null,
  ...
  const openWorker = useCallback((id) => {
    ...
    if (typeof window !== "undefined" && window.innerWidth >= 1024) {
      requestAnimationFrame(() => {
        document.querySelector("[data-detail-scroll]")?.scrollTo({ top: 0, behavior: "smooth" });
      });
    }
  }, []);
  ...
  const addPayment = useCallback((payment) => {
    setWorkers((prev) => prev.map((w) => {
      if (w.id !== selectedId) return w;
      return { ...w, payments: [...(w.payments || []), { ...payment, id: `P-${Date.now()}` }] };
    }));
  ...
  const goPrevMonth = useCallback(() => {
    setViewMonthState((m) => {
      if (m === 0) { setViewYearState((y) => y - 1); return 11; }
      return m - 1;
    });
  }, []);
  const goNextMonth = useCallback(() => {
    setViewMonthState((m) => {
      if (m === 11) { setViewYearState((y) => y + 1); return 0; }
      return m + 1;
    });
  }, []);
  ...
  const prevWeek = useCallback(() => setWeekIndexState((i) => Math.max(0, i - 1)), []);
  const nextWeek = useCallback(() => setWeekIndexState((i) => Math.min(yearWeeks.length - 1, i + 1)), [yearWeeks.length]);

  const markAllPresent = useCallback(() => {
    setWorkers((prev) => prev.map((w) => w.attendance[TODAY] ? w : { ...w, attendance: { ...w.attendance, [TODAY]: "PRESENT" } }));
  }, []);

  const openAddTimeEntry = useCallback(() => { setEditingEntry(null); setShowTimeEntryModal(true); }, []);
  const openEditTimeEntry = useCallback((entry) => { setEditingEntry(entry); setShowTimeEntryModal(true); }, []);
  ...
  const printMonthlyHours = useCallback(() => {
    ...
    if (!win) return;
    let totalHours = 0, totalExtra = 0, totalPay = 0;
    const rows = [...monthlyEntries].sort((a, b) => a.date.localeCompare(b.date)).map((e) => {
      ...
      const totalH = regH + (e.extraHours || 0);
      const pay = Math.round(totalH * selected.hourlyRate);
      totalHours += regH; totalExtra += e.extraHours || 0; totalPay += pay;
    ...
    win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${selected.firstName} ${selected.lastName} — ${MONTH_NAMES[viewMonth]} ${viewYear}</title>
  ...
  return (
    <div className="h-screen w-full flex flex-col lg:flex-row overflow-hidden" style={{ background: "var(--bg)" }}>
      ...
      <div
        className={`${screen === "list" ? "flex" : "hidden"} lg:flex w-full lg:w-96 xl:w-[420px] flex-col shrink-0 h-full border-b lg:border-b-0 lg:border-r`}
        style={{ borderColor: "var(--border)", background: "var(--bg)" }}
      >
        <ListScreen
          workers={workers} filtered={filtered}
          search={search} setSearch={setSearchState}
          roleFilter={roleFilter} setRoleFilter={setRoleFilterState}
          statusFilter={statusFilter} setStatusFilter={setStatusFilterState}
          roles={roles}
          todayPresent={todayPresent} todayAbsent={todayAbsent} todayNotSet={todayNotSet}
          vKey={vKey} orders={orders}
      ...
      <div className={`${screen === "detail" ? "flex" : "hidden"} lg:flex flex-1 flex-col min-w-0 h-full`}>
        <DetailScreen
          selected={selected}
          activeTab={activeTab} setActiveTab={setActiveTabState}
          onBack={goBack}
          year={year} weekIndex={weekIndex} yearWeeks={yearWeeks} currentWeek={currentWeek}
          viewMonth={viewMonth} viewYear={viewYear} vKey={vKey} orders={orders}
          onYearChange={setYear} onPrevWeek={prevWeek} onNextWeek={nextWeek}
          onJumpToToday={goTodayWeek}
          onPrevMonth={goPrevMonth} onNextMonth={goNextMonth} onTodayMonth={goCurrentMonth}
          onAttendanceClick={setAttendance}
          weekRangeLabel={weekRangeLabel}
          monthlyEarned={monthlyEarned} monthlyPaid={monthlyPaid} monthlyBalance={monthlyBalance}
          monthlyEntries={monthlyEntries} monthlyMetersData={monthlyMetersData}
          onAddTimeEntry={openAddTimeEntry} onEditTimeEntry={openEditTimeEntry}
          onDeleteTimeEntry={deleteTimeEntry} onPrint={printMonthlyHours}
          onAddPayment={openAddPayment} onDeletePayment={deletePayment}
        />
      </div>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div
            ...
            style={{ background: "var(--surface)", border: "1px solid var(--border)" }}
          >
            <h3 className="text-lg font-bold mb-4" style={{ color: "var(--ink)" }}>Record Payment</h3>
            <PaymentForm onSave={addPayment} onCancel={() => setShowPaymentModal(false)} />
          </div>
        </div>
      )}
    </div>
